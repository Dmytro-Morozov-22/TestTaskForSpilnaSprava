	...
	public void createTeam(File file, Tournament tournament) throws Exception {
		...
		while ((line = br.readLine()) != null) {
			...
			tournament.getPlayerList().get(i)
					.setRebounds(Integer.parseInt(c[4]));
			tournament.getPlayerList().get(i)
					.setAssists(Integer.parseInt(c[5]));
			tournament.getPlayerList().get(i)
					.setScoredPoints(Integer.parseInt(c[6]));
...
//Counting rating points of each player		
		for (Player player : playerList) {
			player.setRatingPoints(player.getRebounds() * 2
					+ player.getAssists() * 1 + player.getScoredPoints() * 1);
		}
